...
size_t write_callback(char *ptr, size_t size, size_t nmemb, char **data)
{
    ...
    *data = realloc(*data, realsize + 1);
    if (*data == NULL) {
        printf("Unfortunately the Memory allocation failed.\n");
        return 0;
    }
...
int main()
{
    ...
    char *city = "KOLKATA"; // Replace with the required city name
    char url[100];

    // Creating URL for the API request
    sprintf(url, "da3fee2224b3a8feb1119e54be241577
, city); // Replace with your API key
